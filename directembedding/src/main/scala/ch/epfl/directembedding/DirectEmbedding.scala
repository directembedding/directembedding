...
protected[directembedding] object Macros {
  ...
  def inlineMethod(c: Context)(f: c.Tree, args: List[c.Tree]): c.Tree = {
    ...
    println(showRaw(f, printIds = true))
    val q"def ${ _ }(..$params): $tpe = $body" = f
    val paramsMap = (params zip args).map {
      case (param @ ValDef(_, paramNme, _, _), arg) =>
        val temp = c.fresh(newTermName(paramNme.toString))
  ...
  def extractMethodTree(c: Context)(x: c.Expr[Any]): c.Expr[MethodTree] = {
    ...
    println(showRaw(q"$defTree", printIds = true))
    println(showRaw(c.typecheck(q"$defTree"), printIds = true))
